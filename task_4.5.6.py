...
with webdriver.Chrome() as browser:# Инициализация драйвера в контексте with, чтобы он закрылся после завершения работы
    browser.get('https://parsinger.ru/selenium/5/5.html')# Открываем URL
    boxes = browser.find_elements(By.CLASS_NAME, 'check') # ищем все коробки с class='check'
    # перебираем boxes, получаем значение атрибута 'value' и если значение есть в списке numbers кликаем box
    [box.click() for box in boxes if int(box.get_attribute('value')) in numbers]
    browser.find_element(By.CLASS_NAME, 'btn').click() # находим кнопку class='btn' и кликаем уё
    print(browser.find_element(By.ID, 'result').text) # находим и выводим на экран id='result'