...
with webdriver.Chrome() as browser:  # Инициализация драйвера в контексте with, закроется после завершения работы
    ...
    def get_code(size_x, size_y):
        """
        Функция перебирает элементы списков, задаёт размеры окна и возвращает
        текущие размеры окна при появлении числового пароля в элементе с id='result'
        """
        for x in size_x:
            for y in size_y:
    ...
    print(get_code(window_size_x, window_size_y))  # размер рабочей зоны
    print(browser.get_window_size())  # размер всего окна
